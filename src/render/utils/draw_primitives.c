...
/*   Updated: 2025/06/06 19:05:20 by seungah       ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line(mlx_image_t *img, t_point start, t_point end, int color)
{
	...
	while (steps-- >= 0)
	{
		if (is_coordinate_valid((int)x, (int)y, img->width, img->height))
...
void	draw_cell(mlx_image_t *img, t_cell cell)
{
	...
	while (x < cell.size)
	{
		...
		while (y < cell.size)
		{
			if (is_coordinate_valid(cell.px + x, cell.py + y, img->width,
					img->height))
