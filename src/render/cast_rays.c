...
/*   Updated: 2025/05/20 17:52:57 by jsong         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
bool	check_hit_wall(t_map *map, t_ray *ray)
{
	...
	point.x = ray->end_x / MINIMAP_CELL_SIZE;
	point.y = ray->end_y / MINIMAP_CELL_SIZE;
	printf("Checked point is (%d, %d) \n", point.x, point.y);
	if (is_valid_point(map, point.x, point.y)
		&& map->grid[point.y][point.x] == '0')
		return (false);
	// if (map->grid[my][mx] == '1')
	// 	return (true);
	return (true);
